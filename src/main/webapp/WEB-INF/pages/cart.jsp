            ...
            <tr>
              ...
              <td class="quantity">
                  ...
                  <c:set var="error" value="${errors[item.product.id]}"/>
                    ...
                    <input type="hidden" name="productId}" value="${item.quantity}"/>
              </td>
              <td>
              <a href="${pageContext.servletContext.contextPath}/products/priceHistory/${item.product.id}">
                <fmt:formatNumber value="${item.product.price}"
                                  type="currency"
                                  currencySymbol="${item.product.currency != null ? item.product.currency.symbol : '$'}"
                                  pattern="#,##0.00"/>
              </a>
              </td>
              <td><button form ="deleteCartItem"
              formaction="${pageContext.servletContext.contextPath}/cart/deleteCartItem/${item.product.id}" >Delete<button></td>
            </tr>
          </c:forEach>
          <tr>
          <td>Total cost</td>
